/**
 * Created by nitish.thakur on 21/03/16.
 */
...
function openDatabase() {
    ...
    indexedDBOpenRequest.onupgradeneeded = function(e) {
        ...
        //store.createIndex('index_on_tote_id', 'tote_id', {unique: true, multiEntry: false});
    };
...
} // close of openDatabase function



function getValueFromDatabase(key) {
    var objectStore = getObjectStore(STORE_NAME, IDBTransaction.READ_ONLY);
...
function putValueInDatabase(key, value) {
    // var txn = idbDatabase.transaction(STORE_NAME, IDBTransaction.READ_WRITE);
    var objectStore = getObjectStore(STORE_NAME, IDBTransaction.READ_WRITE);
...
self.addEventListener('fetch', function(event) {
    if (event.request.method == 'GET') {
        ...
        var toteResponse = getValueFromDatabase(event.request.url);
        if(!toteResponse) { // toteDetails are not present in indexeddb
            return fetch(event.request.clone()).then(function(response) {     // use fetch(url, {credentials: 'include'}) for passing user credentials
                if (response.status < 400) {
                    putValueInDatabase(toteId, response.clone());
                }
                return response;
            }).catch(function(error) {
                // The catch() will be triggered for network failures.
                // throw error;
                return toteResponse;  // return from indexedDB.
            }
...
function replayToteRequests() {
    ...
    getObjectStore(STORE_NAME).openCursor().onsuccess = function(event) {
        ...
        if (cursor) {
            ...
            console.log('About to replay %d saved tote requests...',
